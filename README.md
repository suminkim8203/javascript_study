...
## 10.1.2 매개변수(파라미터)와 함수 값 반환

- 함수 정의에서는 매개변수(parameter)를 통하여 함수를 호출한 곳에서 사용되는 변수나 데이터 값을 전달 받는다.
...
function circleArea(r) {
  // 매개변수 r: 함수 호출에 사용된 변수나 데이터 값, 즉 인수(argument)를 전달 받는 것에 사용
...
- 함수 정의가 먼저 이루어져야 하기 때문에 호이스팅 된다. (그러나 익명함수의 경우 안 됨.)
- 먼저 함수 정의, 선언부터 하는 프로그래밍을 습관하 하자.
